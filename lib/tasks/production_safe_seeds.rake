# lib/tasks/production_safe_seeds.rake

namespace :db do
  desc "Seed sample data for any environment (USE WITH CAUTION IN PRODUCTION)"
  task seed_sample_data: :environment do
    puts "Preparing to seed sample data in the #{Rails.env} environment."
    puts "This will create fake users, groups, and expenses in your database."
    puts "Are you absolutely sure you want to continue? (y/n)"
    ...
    created_users = users.map do |user_data|
      User.find_or_create_by!(email: user_data[:email]) do |user|
        user.password = "password123" # In real production, use a secure random password
    ...
    puts "Sample data seeding completed successfully in the #{Rails.env} environment!"
  end
